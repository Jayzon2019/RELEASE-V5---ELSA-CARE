...
import { Enumerations } from '../common/enumerations';
...
export class ApplyComponent implements OnInit {
  ...
  constructor(private router: Router, private location: Location,
    ...
    public session: SessionStorageService, private ngxService: NgxUiLoaderService, private vps: ViewportScroller) {
    this.ngxService.start();
    this.getQuoteFormData = JSON.parse(this.session.get("getQuoteForm") || "[]");
...
submitApplyForm()
{
	...
	if (this.getApplyForm.valid)
	{
		...
		this.session.set("extensionData", JSON.stringify(this.dynamicArray));
		...
		const getApplyFormData = JSON.parse(this.session.get("getApplyForm") || "[]");
		const getQuoteForm = JSON.parse(this.session.get("getQuoteForm") || "[]");
