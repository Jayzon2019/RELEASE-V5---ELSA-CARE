	...
	public class CustomTicketStore : ITicketStore
	{
		...
		public async Task RemoveAsync(string key)
		{
			...
			if (Guid.TryParse(key, out var id))
			{
				...
				//THIS IS A HACK: Find out why IdentityServer doesn't delete the session
				var model = userSessionRepository.Get(id);
				if (model != null)
				{
					var models = userSessionRepository
						.GetAll()
						.Where(session => session.User.Id == model.User.Id)
						.ToList();

					if (models.Count > 0)
					{
						userSessionRepository.Delete(models);
					}
				}
			}
		}
