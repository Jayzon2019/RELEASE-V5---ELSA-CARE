...
namespace InLife.Store.Identity.Features
{
	...
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> Login(string returnUrl)
		{
			...
			// Check if there's an existing user in the DB
			// If empty create an admin account
			var isInitialRegistration = this.dbContext.Users.Count() == 0;
			if (isInitialRegistration)
			{
				return RedirectToAction("InitialRegistration", "Account", new { returnUrl });
			}
		...
		/// <summary>
		/// Entry point into the initial registration workflow
		/// </summary>
		[HttpGet]
		public async Task<IActionResult> InitialRegistration(string returnUrl)
		{
			// build a model so we know what to show on the login page
		...
		/// <summary>
		/// Handle postback from new administrator registration
		/// </summary>
		[HttpPost]
		[ValidateAntiForgeryToken]
		public async Task<IActionResult> InitialRegistration(RegistrationViewModel model, string button)
		{
			// check if we are in the context of an authorization request
			var context = await this.interaction.GetAuthorizationContextAsync(model.ReturnUrl);
			...
			if (ModelState.IsValid)
			{
				...
				if (createResult.Succeeded)
				{
					...
					await this.signInManager.SignInAsync(user, isPersistent: model.RememberLogin);
					await this.events.RaiseAsync(new UserLoginSuccessEvent(user.UserName, user.Id, user.UserName, clientId: context?.Client.ClientId));
				...
				await this.events.RaiseAsync(new UserLoginFailureEvent(model.Username, "error creating the admin account", clientId: context?.Client.ClientId));
				ModelState.AddModelError(string.Empty, AccountOptions.InvalidCredentialsErrorMessage);
			}

			// something went wrong, show form with error
			var vm = await BuildRegistrationViewModelAsync(model);
			return View(vm);
		}
		...
		private async Task<RegistrationViewModel> BuildRegistrationViewModelAsync(LoginInputModel model)
		{
			var vm = await BuildRegistrationViewModelAsync(model.ReturnUrl);
			vm.Username = model.Username;
			vm.RememberLogin = model.RememberLogin;
			return vm;
		}
